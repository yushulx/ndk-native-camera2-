...
//      camera_status_t status = ACaptureRequest_addTarget(req.request_, req.yuvTarget);
//      ACaptureSessionOutput_create(req.yuvWindow, &req.yuvOutput);
...
//                           ACAMERA_CONTROL_AE_MODE, 1, &aeModeOff));
//  CALL_REQUEST(setEntry_i32(requests_[PREVIEW_REQUEST_IDX].request_,
//                           ACAMERA_CONTROL_AE_TARGET_FPS_RANGE, 2, fps));

//  CALL_REQUEST(setEntry_u8(requests_[YUV_REQUEST_IDX].request_,
//                           ACAMERA_CONTROL_AE_MODE, 1, &aeModeOff));
